"""Build the 10-model paper comparison including TEG-concat fusion models."""
...
DEFAULT_FUSION_NODE = (
    OUTPUTS / "models" / "paper_teg_concat_fusions_splitB_10seed_summary.csv"
)
DEFAULT_FUSION_OD = (
    OUTPUTS
    / "od_paths"
...
DEFAULT_OUT = (
    OUTPUTS / "pyvrp_cvrp" / "paper_model_comparison_with_fusion_10seed"
)

FUSION_MODEL_DISPLAY = {
    **base.MODEL_DISPLAY,
...
PAIRWISE_TARGETS = [
    ("GraphSAGE-TEG-Concat", "GraphSAGE", "TEG enhancement over GraphSAGE"),
    ("SGFormer-TEG-Concat", "SGFormer-adapted", "TEG enhancement over SGFormer"),
    ("GraphSAGE-TEG-Concat", base.STABLE, "Fusion vs current main model"),
    ("SGFormer-TEG-Concat", base.STABLE, "Fusion vs current main model"),
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fusion-node", type=Path, default=DEFAULT_FUSION_NODE)
    ...
    parser.add_argument("--fusion-od", type=Path, default=DEFAULT_FUSION_OD)
    ...
    parser.add_argument("--fusion-self-summary", type=Path, default=DEFAULT_FUSION_SELF)
    parser.add_argument("--common-summary", type=Path, default=DEFAULT_COMMON_WITH_FUSION)
    parser.add_argument("--load-summary", type=Path, default=DEFAULT_LOAD_WITH_FUSION)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--bootstrap-samples", type=int, default=10000)
    return parser.parse_args()


def install_fusion_globals() -> None:
    base.MODEL_DISPLAY.clear()
    base.MODEL_DISPLAY.update(FUSION_MODEL_DISPLAY)
    base.MODEL_ORDER[:] = FUSION_MODEL_ORDER


def fusion_node_table(rows: list[dict[str, str]]) -> list[dict[str, str]]:
...
def fusion_pairwise_significance(
    ...
    for customer_set in ("A", "B"):
        for metric, keyed in metric_sources:
            ...
            for target, reference, purpose in PAIRWISE_TARGETS:
                ...
                low, high = base.bootstrap_ci(diff, samples, seed=20260710)
                block.append(
                    {
                        "customer_set": customer_set,
                        "metric": metric,
...
def append_fusion_report(path: Path, pairwise_rows: list[dict[str, object]]) -> None:
    lines = [
        "",
...
def main() -> None:
    ...
    fusion_sig_rows = fusion_pairwise_significance(
    ...
    base.write_csv(args.output_dir / "fusion_pairwise_significance.csv", fusion_sig_rows)
    ...
    append_fusion_report(report_path, fusion_sig_rows)
    ...
    integrity = {
        ...
        "fusion_pairwise_significance_rows": len(fusion_sig_rows),
        "all_models_summary_rows": len(model_summary),
        "models": FUSION_MODEL_ORDER,
    ...
    print(f"Wrote fusion comparison package to {args.output_dir}")
